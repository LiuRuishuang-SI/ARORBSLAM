...
inline Eigen::Matrix4f ToMat4f(const cv::Mat &m)
{
    ...
    if (m.type() == CV_32F)
    {
        for (int r = 0; r < 4; ++r)
            for (int c = 0; c < 4; ++c)
                T(r, c) = m.at<float>(r, c);
    }
    else
    {
        for (int r = 0; r < 4; ++r)
            for (int c = 0; c < 4; ++c)
                T(r, c) = static_cast<float>(m.at<double>(r, c));
    }
    return T;
}
...
// ---------------- Dump utilities ----------------
static bool SavePlaneTXT(const std::string &path, const Eigen::Vector4d &plane)
{
...
int main(int argc, char **argv)
{
    ...
    fs::create_directories("../output/points");
    fs::create_directories("../output/plane");
    ...
    for (size_t i = 0; i < imgs[0].size(); ++i)
    {
        ...
        if (pts_world.size() >= 50)
        {
            ...
            std::cout << "[Plane] a,b,c,d = " << plane.transpose()
                      << " | inliers " << inliers.size()
                      << "/" << pts_world.size() << std::endl;

            char fp_csv[256], fp_txt[256];
            std::snprintf(fp_csv, sizeof(fp_csv), "../output/points/%06zu.csv", i);
            std::snprintf(fp_txt, sizeof(fp_txt), "../output/plane/%06zu.txt", i);
            SaveLabeledPointsCSV(fp_csv, pts_world, inliers, plane);
            SavePlaneTXT(fp_txt, plane);
        }
        else
        {
